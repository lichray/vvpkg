...
int BLAKE2b_Init(BLAKE2B_CTX *c, size_t outlen)
{
    BLAKE2B_PARAM P[1];
                            ...
                            size_t len)
{
    ...
    do {
        for (i = 0; i < 16; ++i) {
            m[i] = load64(blocks + i * int(sizeof(m[i])));
        }
...
{
    const uint8_t *in = reinterpret_cast<uint8_t const*>(data);
    size_t fill;
 ...
 * Always returns 1.
 */
int BLAKE2b_Final(unsigned char *md, BLAKE2B_CTX *c, size_t outlen)
{
